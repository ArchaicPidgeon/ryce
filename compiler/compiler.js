...
(function() {
  ...
  var buildPage, compile, forbiddenWordsRegex, format, func, generate, getOptions, isArray, isString, la, la_old, log, logAll, logs, nameRegexA, nameRegexB, numRegexA, numRegexB, operatorRegex, pVal, parse, reservedWordRegex, run, ryce, test, tokenize, translate,
    indexOf = [].indexOf;
  ...
  tokenize = function(charStream, logMode) {
    var advance, eat, i, idx, indentStack, len, litMode, scanToken, scanTokenBlock, token, tokenFuncs, tokenStream;
    if (charStream.match(/[ ]?[^\n ]/)[0].length > 1) {
    ...
    tokenFuncs.commentBlockA = function() {
      return scanTokenBlock('comment', "^###");
    };
    tokenFuncs.commentBlockB = function() {
      return scanTokenBlock('comment', "^///");
    };
    tokenFuncs.commentLineA = function() {
  ...
  getOptions = function(code) {
    var i, key, len, options, pair, res, val;
    res = code.match(/^[ \n]*### ryce [\s\S]*?\n###/);
    if (res) {
      code = code.slice(res[0].length - 3);
      res = res[0].split('\n').filter(function(arg) {
        return indexOf.call(arg, ':') >= 0;
      });
    } else {
      res = code.match(/^([ \n]*\[.*?\])*/)[0].slice(1, -1).split(/\][ \n]*\[/);
    }
    ...
    for (i = 0, len = res.length; i < len; i++) {
      ...
      if (key === 'script' || key === 'style' || key === 'macro') {
  ...
  buildPage = function(code, options) {
    var i, j, len, len1, ref, ref1, res, script, style;
    res = ["<!doctype html>"];
    res.push(`<title>${options.title || "untitled"}</title>`);
    ref = options.style;
    for (i = 0, len = ref.length; i < len; i++) {
      style = ref[i];
  ...
  //#########################################################################################
  test = `[mode: html] [title: test] [script: index.zq]
