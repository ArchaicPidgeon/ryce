...
(function() {
  ...
  var fn, forbiddenWordsRegex, generate, isArray, isString, la, la_old, log, logAll, logs, nameRegexA, nameRegexB, numRegexA, numRegexB, operatorRegex, pVal, parse, reservedWordRegex, run, ryce, tokenize, translate,
    indexOf = [].indexOf;
  ...
  reservedWordRegex = RegExp(`^(def|await|yield|outer|var|let|return|break|continue|comment|lang|default|while|until|loop|for|of|in|ever|if|unless|else|opt|alt|do|try|catch|finally|debugger)${la}`);

  forbiddenWordsRegex = RegExp(`^[.]?(class|inherits|case|new|delete|function|instanceof|typeof|switch|typeof|void|with|const|export|import|super|let|static|enum|implements|interface|package|private|protected|public)${la}`);

  nameRegexA = RegExp(`^~?(@?[a-zA-Z_$][a-zA-Z0-9_$]*|&[a-zA-Z_$][a-zA-Z0-9_$]*|&[0-9]+|&&|&|@)((:|::)[a-zA-Z_$][a-zA-Z0-9_$]+|:[0-9]+)*[?!]?${la}`);

  nameRegexB = RegExp(`^([.:][a-zA-Z_$][a-zA-Z0-9_$]*)((:|::)[a-zA-Z_$][a-zA-Z0-9_$]+|:[0-9]+)*${la}`);

  //#########################################################################################
  tokenize = function(charStream, logMode) {
    var advance, eat, i, idx, indentStack, len, scanToken, scanTokenBlock, token, tokenFuncs, tokenStream;
    if (charStream.match(/[ ]?[^\n ]/)[0].length > 1) {
    ...
    tokenFuncs.key = function() {
      return scanToken('key', '^[a-zA-Z_$][a-zA-Z0-9_$]*[:;]' + la);
    };
    ...
    tokenFuncs.stringWordA = function() {
  ...
  parse = function(tokenStream) {
    ...
    parseLineFunction = function() {
      ...
      if ((canStartHead(token)) || ((ref = token.type) === 'dotName' || ref === 'biOp' || ref === 'pipe')) {
    ...
    parseStatementBlock = function() {
      ...
      while (true) {
        ...
        } else if (token.content === 'debugger') {
          content.push(parseTerminal());
    ...
    parseFail = function(fail) {
      return fail + '\n' + JSON.stringify(tokenStream.slice(idx - 1, +(idx + 5) + 1 || 9e9));
    };
    parseAssignemnt = function() {
      ...
      parseTerminal();
      if (!canStartHead(token)) {
        throw parseFail('value expected');
      }
      return {
  ...
  ryce = function(ry, js) {
    var err, i, len, obj, recipe, res, step;
    recipe = [tokenize, parse, translate];
    obj = {};
    res = {};
    try {
      eval(js);
